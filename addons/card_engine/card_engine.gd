...
func _init():
	_library.load_from_database(CEInterface.card_database_path())
...
func final_text(card_data, text):
	...
	if card_data.targets == "single":
		final_text = final_text.replace("$targets", "a " + card_data.targets + " target")
	elif card_data.targets == "enemies" or card_data.targets == "enemies":
		final_text = final_text.replace("$targets", "all " + card_data.targets)
	else:
		final_text = final_text.replace("$targets", card_data.targets)
	return final_text
